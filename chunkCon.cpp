...
int handleChunkedRequest(clientData* cData, Server& server)
{
    ...
    {
        size_t slash = cData->filename.find_last_of('/');
        if (slash != std::string::npos)
            ensureDirExists(cData->filename.substr(0, slash));
    }
    ...
    {
        perror("open()");
        cData->response_data = sendErrorCode(500, server, cData);
    ...
    while (cData->request_data.length())
    {
        ...
        {
            cData->response_data = sendErrorCode(413, server, cData);
            cData->state = SENDING_RESPONSE;
            close(cfd);
            return (2);
        }
...
int handleContentLengthRequest(clientData* cData, Server& server)
{
    ...
    if (cData->filename.empty())
    {
        std::string swpDir = server._serverRoot.substr(1) + "/swp";
        if (!ensureDirExists(swpDir))
        {
            cData->response_data = sendErrorCode(500, server, cData);
            cData->state = SENDING_RESPONSE;
            return (2);
        }
        cData->filename = swpDir + "/.clength_" + to_string(clIndex++) + ".swp";
