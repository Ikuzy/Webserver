                        ...
                        if (cData->headers_complete || cData->ccFlag == 0)
                        {
                            if (cData->is_chunked)              // waitChunkedBody()
                            {
                                ...
                                else if (header_end != std::string::npos && !cData->flag)
                                {
                                    ...
                                    {
                                        size_t slash = fileStr.find_last_of('/');
                                        if (slash != std::string::npos)
                                            ensureDirExists(fileStr.substr(0, slash));
                                    }
                                    ...
                                    if (cfd < 0)
                                    {
                                        ...
                                        epoll_ctl(epoll_fd, EPOLL_CTL_MOD, cliefd, &modify_event);
